...
# ============================================
# Instant Response Endpoints (Pre-loaded Data)
# ============================================

@app.get("/moods/full")
def get_moods_full(country: str = "US", language: str = "en"):
    """
    Get ALL moods data with playlists AND tracks pre-loaded.
    Returns: { "카테고리명": { playlists: [{ playlistId, title, thumbnails, tracks: [...] }] } }
    This enables instant response on category click AND playlist click.
    """
    cache_key = make_cache_key("moods_full", country, language)

    # Check Redis cache first
    cached = cache_get(cache_key)
    if cached is not None:
        print(f"[REDIS HIT] /moods/full country={country}")
        return cached

    print(f"[REDIS MISS] /moods/full country={country} - building full data...")

    try:
        yt = get_ytmusic(country=country, language=language)

        # Step 1: Get mood categories
        moods_data = run_with_retry(yt.get_mood_categories)
        if not moods_data:
            return {}

        result = {}

        # Step 2: For each category, get playlists with tracks
        for section_name, categories in moods_data.items():
            if not isinstance(categories, list):
                continue
            ...
            for cat in categories:
                ...
                try:
                    playlists = None
                    try:
                        playlists = run_with_retry(yt.get_mood_playlists, params)
                    except KeyError:
                        playlists = parse_genre_playlists(yt, params)

                    if playlists and isinstance(playlists, list):
                        # For each playlist, get tracks
                        for playlist in playlists[:10]:  # Limit to 10 playlists per category
                            if not isinstance(playlist, dict):
                                continue
                            playlist_id = playlist.get("playlistId")
                            if not playlist_id:
                            ...
                            try:
                                watch_data = run_with_retry(yt.get_watch_playlist, playlistId=playlist_id)
                                if watch_data and "tracks" in watch_data:
                                    playlist_data["tracks"] = watch_data["tracks"][:50]  # Limit to 50 tracks
                            except Exception as e:
                                print(f"Error fetching tracks for {playlist_id}: {e}")

                            category_data["playlists"].append(playlist_data)
                except Exception as e:
                    print(f"Error fetching playlists for category {title}: {e}")

                result[section_name].append(category_data)

        # Store in Redis cache (48시간 TTL)
        cache_set(cache_key, result, TTL_MOOD_PLAYLISTS)

        print(f"[MOODS FULL] Built full data for {country}")
        return result

    except Exception as e:
        print(f"Error in get_moods_full: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/charts/full")
def get_charts_full(country: str = "US", language: str = "en"):
    """
    Get charts data WITH playlist tracks pre-loaded.
    Returns: { artists: [...], playlists: { topSongs: {tracks: [...]}, topVideos: {tracks: [...]} } }
    This enables instant response on chart card click.
    """
    cache_key = make_cache_key("charts_full", country, language)

    # Check Redis cache first
    cached = cache_get(cache_key)
    if cached is not None:
        print(f"[REDIS HIT] /charts/full country={country}")
        return cached

    print(f"[REDIS MISS] /charts/full country={country} - building full data...")

    try:
        yt = get_ytmusic(country=country, language=language)

        # Get charts (artists)
        charts_data = run_with_retry(yt.get_charts, country=country)

        # Get chart playlist IDs
        chart_config = CHART_CONFIGS.get(country, CHART_CONFIGS.get("ZZ", {}))

        result = {
            "artists": charts_data.get("artists", {}).get("results", []) if charts_data else [],
            "playlists": {}
        }

        # Pre-load each chart playlist's tracks
        for chart_type in ["topSongs", "topVideos"]:
            playlist_id = chart_config.get(chart_type)
            if playlist_id:
                try:
                    watch_data = run_with_retry(yt.get_watch_playlist, playlistId=playlist_id)
                    result["playlists"][chart_type] = {
                        "playlistId": playlist_id,
                        "tracks": watch_data.get("tracks", [])[:100] if watch_data else []
                    }
                except Exception as e:
                    print(f"Error fetching {chart_type} for {country}: {e}")
                    result["playlists"][chart_type] = {"playlistId": playlist_id, "tracks": []}

        # Store in Redis cache (24시간 TTL)
        cache_set(cache_key, result, TTL_CHARTS)

        print(f"[CHARTS FULL] Built full data for {country}")
        return result

    except Exception as e:
        print(f"Error in get_charts_full: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/home/full")
def get_home_full(limit: int = 100, country: str = "US", language: str = "en"):
    """
    Get home feed WITH album/playlist tracks pre-loaded.
    Each item with browseId (album) or playlistId gets its tracks pre-loaded.
    This enables instant response on any home banner click.
    """
    cache_key = make_cache_key("home_full", limit, country, language)

    # Check Redis cache first
    cached = cache_get(cache_key)
    if cached is not None:
        print(f"[REDIS HIT] /home/full country={country}")
        return cached

    print(f"[REDIS MISS] /home/full country={country} - building full data...")

    try:
        yt = get_ytmusic(country=country, language=language)

        # Get home feed
        home_data = run_with_retry(yt.get_home, limit=limit)
        if not home_data or not isinstance(home_data, list):
            return []

        # Pre-load tracks for each item
        for section in home_data:
            if not section or not isinstance(section, dict):
                continue
            contents = section.get("contents", [])
            for item in contents:
                ...
                # Pre-load album tracks
                browse_id = item.get("browseId")
                if browse_id and browse_id.startswith("MPREb"):
                    try:
                        album_data = run_with_retry(yt.get_album, browse_id)
                        if album_data and "tracks" in album_data:
                            item["_preloaded_tracks"] = album_data.get("tracks", [])
                            item["_preloaded_album"] = {
                                "title": album_data.get("title"),
                                "artists": album_data.get("artists"),
                                "thumbnails": album_data.get("thumbnails"),
                            }
                    except Exception as e:
                        print(f"Error preloading album {browse_id}: {e}")
                ...
                if playlist_id:
                    try:
                        watch_data = run_with_retry(yt.get_watch_playlist, playlistId=playlist_id)
                        if watch_data and "tracks" in watch_data:
                            item["_preloaded_tracks"] = watch_data.get("tracks", [])[:50]
                    except Exception as e:
                        print(f"Error preloading playlist {playlist_id}: {e}")

        # Store in Redis cache (24시간 TTL)
        cache_set(cache_key, home_data, TTL_HOME)

        print(f"[HOME FULL] Built full data for {country}")
        return home_data

    except Exception as e:
        print(f"Error in get_home_full: {e}")
        raise HTTPException(status_code=500, detail=str(e))
